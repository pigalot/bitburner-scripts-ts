...
class TargeterDeamon extends Deamon {
    ...
    public async run(detail: boolean) {
        ...
        const maxCost = this.maxRamToUse * this.resourceManagerData.totalRam;
        for (const target of servers) {
			...
			do {
                ...
                while (hN == 1 && (ret.ramCost / maxCost > 1.0)) { //hN 1 profile to expensive, lengrhen cL to reduce cost
					await this.ns.sleep(0);
					ret = this.estimateBestScore(target, hN, player, Math.ceil(ret.calculatedBufferTime * ret.ramCost / maxCost));
				}
    ...
    private estimateBestScore(target: ServerData, hackThreads: number, player: Player, bufferTime = this.minBufferTime, cores = 1): Batch {
        ...
        const growThreads = Math.max(1, Math.ceil(this.threadBufferMultiplier * growAnalyzeHackAmount(target.server, player, amountHacked, this.gameInfoData.bitNodeMultipliers?.ServerGrowthRate ?? 1, postHackSecurity))); //number of Grow threads needed to offset each hack
